...
arduino = serial.Serial('COM6', 9600)  # Replace 'COM_PORT' with your actual COM port
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        for (ex, ey, ew, eh) in eyes:
            ...
            if probability_open > 0.2:
                eye_status = "Open"
                arduino.write(b'H')  # Send 'H' when eye is open
                # Show the window if the eye is open
                cv2.resizeWindow('Eye Status Detection', 640, 480)
            else:
                eye_status = "Closed"
                arduino.write(b'L')  # Send 'L' when eye is closed
